...
function drawPacman(){const scale=14,ox=16,oy=16;x.fillStyle='#050505';x.fillRect(0,0,cv.width,cv.height);function px(p,c){x.fillStyle=c;x.fillRect(ox+p.x*scale,oy+p.y*scale,scale,scale)}function wall(p){x.strokeStyle='#245bff';x.lineWidth=3;x.strokeRect(ox+p.x*scale+2,oy+p.y*scale+2,scale-4,scale-4)}S.walls.forEach(wall);const colors=['#ff4f63','#55d7ff','#ff9df2','#ffb447'];S.ghosts.forEach((g,i)=>{let gx=ox+g.x*scale,gy=oy+g.y*scale;x.fillStyle=colors[i%colors.length];x.beginPath();x.arc(gx+scale/2,gy+scale/2,scale*.42,Math.PI,0);x.rect(gx+scale*.08,gy+scale/2,scale*.84,scale*.38);x.fill();x.fillStyle='white';x.fillRect(gx+4,gy+6,3,3);x.fillRect(gx+9,gy+6,3,3)});S.pellets.forEach(p=>{x.fillStyle='#ffe6a6';x.beginPath();x.arc(ox+p.x*scale+scale/2,oy+p.y*scale+scale/2,2.1,0,Math.PI*2);x.fill()});let px0=ox+S.player.x*scale+scale/2,py0=oy+S.player.y*scale+scale/2;x.fillStyle='#ffd21f';x.beginPath();x.moveTo(px0,py0);x.arc(px0,py0,scale*.48,.22*Math.PI,1.78*Math.PI);x.closePath();x.fill();$('score').textContent=S.score;$('steps').textContent=S.steps;$('state').textContent=S.win?'win':(S.dead?'dead':'run');$('msg').textContent=S.msg||'waka waka';$('msg').className='status'+(S.dead?' dead':'')}
