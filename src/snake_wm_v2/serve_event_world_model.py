...
<script>
const MODE="__MODE__",N=16,C=8,cv=document.getElementById('c'),x=cv.getContext('2d'),$=id=>document.getElementById(id),D=[[0,-1],[0,1],[-1,0],[1,0]];if(MODE==='pacman'){cv.width=224;cv.height=248}let S;function pt(x,y){return{x,y}}function same(a,b){return a.x===b.x&&a.y===b.y}function has(a,p){return a.some(q=>same(q,p))}function rm(a,p){let i=a.findIndex(q=>same(q,p));if(i>=0)a.splice(i,1);return i>=0}function inside(p){return p.x>=0&&p.x<N&&p.y>=0&&p.y<N}function add(p,d){return pt(p.x+D[d][0],p.y+D[d][1])}function border(){let w=[];for(let i=0;i<N;i++)w.push(pt(i,0),pt(i,N-1),pt(0,i),pt(N-1,i));return w}
