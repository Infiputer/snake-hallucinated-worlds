...
This makes grounding unnecessary for this setting.

\begin{table}[t]
  \centering
  \input{tables/snake_argmax_randomized_summary.tex}
...
\begin{table}[t]
  \centering
  \input{tables/snake_hard_vs_prob_summary.tex}
...
\begin{table}[t]
  \centering
  \input{tables/pacman_argmax_summary.tex}
...
The intended interpretation is capacity-based.
If a tiny world model produces high hallucinated pellet reward but poor real-simulator pellet reward, then the policy has found a model-specific strategy even under discrete rewards.
If larger world models reduce this transfer gap, then model capacity improves not only frame prediction but also the policy-relevant event dynamics.
This is the central scaling question of the Pac-Man extension.
