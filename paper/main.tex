...
The matched objective sweep trains two policies for each world-model/context/CNN cell.
The first policy optimizes the world model's scalar reward head $\hat{r}_t$.
The second policy optimizes the raw floating-point length-head change $\hat{\ell}_{t+1}-\hat{\ell}_t$ with no length-delta clamp.
The two resulting tables therefore have the same rows and columns, differing only in the PPO objective used during hallucinated training.
...
We also include a smaller fixed-checkpoint pilot with three PPO objectives: predicted reward, predicted length change, and a mixed objective.
The full matched sweep is the main comparison between scalar-reward training and length-head training.
...
The matched objective sweep trains two world-model sizes, three context lengths, and three CNN policy sizes under two PPO objectives, yielding 36 world-model/policy/objective settings.
...
\begin{table}[t]
  \centering
  \input{tables/matched_reward_objective_summary.tex}
  \caption{Matched sweep trained with the world model's scalar reward head. Gap is hallucinated objective return minus real-simulator return.}
  \label{tab:matched-reward}
\end{table}

\begin{table}[t]
  \centering
  \input{tables/matched_length_objective_summary.tex}
  \caption{Matched sweep trained with the unclamped floating-point length-head change $\hat{\ell}_{t+1}-\hat{\ell}_t$. Rows and columns match Table~\ref{tab:matched-reward}.}
  \label{tab:matched-length}
\end{table}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/matched_objective_context_scaling.png}
  \caption{Real-simulator apple count as context length changes, averaged across CNN sizes. Separate curves compare scalar-reward PPO and unclamped length-head PPO for each world-model size.}
  \label{fig:objective-context-scaling}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\linewidth]{figures/matched_objective_policy_scaling.png}
  \caption{Real-simulator apple count as CNN policy size changes, averaged across world-model sizes and contexts.}
  \label{fig:objective-policy-scaling}
\end{figure}

\begin{table}[t]
  \centering
  \input{tables/matched_objective_comparison.tex}
  \caption{Per-cell real-apple comparison between scalar-reward PPO and unclamped length-head PPO. Positive delta means scalar-reward PPO transferred better.}
  \label{tab:matched-objective-comparison}
\end{table}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\linewidth]{figures/matched_objective_delta_heatmap.png}
  \caption{Real-apple advantage of scalar-reward PPO over unclamped length-head PPO for each matched world-model/context/CNN cell.}
  \label{fig:objective-delta-heatmap}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.58\linewidth]{figures/matched_objective_scatter.png}
  \caption{Matched-cell scatter plot comparing real apples after scalar-reward PPO and unclamped length-head PPO. The diagonal indicates equal transfer.}
  \label{fig:objective-scatter}
\end{figure}
...
\begin{table}[t]
  \centering
  \input{tables/scalar_consistency_summary.tex}
...
\begin{table}[t]
  \centering
  \input{tables/reward_objective_ablation.tex}
